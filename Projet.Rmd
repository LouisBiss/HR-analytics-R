...
```{r}
#df_average_working_time <- mean.Date(df_working_time_by_day)
df_average_working_time <- data.frame(matrix(nrow=nrow(df_working_time_by_day)))
for (i in 1:ncol(df_working_time_by_day)) {
  df_average_working_time$mean<- mean(df_working_time_by_day[,i],na.rm = TRUE)
}
df_average_working_time   
```
...
df_avg <- mean.Date(df_working_time_by_day, na.rm = TRUE)
